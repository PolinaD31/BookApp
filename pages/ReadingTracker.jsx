...
import { getStartOfWeek, changeWeek, calculateWeeklyReading, formatDate, getEndOfWeek} from '../utils/weekHelper';
...
export default function ReadingTracker () {
  ...
  useEffect(() => {
    onValue(ref(database, `readingData/${auth.currentUser.uid}`), (snapshot) => {
      if (snapshot.exists()) {
    ...
    onValue(ref(database, `readingGoals/${auth.currentUser.uid}`), (snapshot) => {
      if (snapshot.exists()) {
  ...
  return (
    <View style={styles.container} behavior='padding'>
      ...
      {goalData[currentWeekStart] && 
        <GoalProgressBar weeklyReading={weeklyReading} goal={goalData[currentWeekStart].goal} />
      }
      </View>
      <View style={readingTrackerStyles.dateContainer}>
        <Text variant='titleLarge' style={{color: purple}}>{formatDate(currentWeekStart)} - {formatDate(weekEnd)}</Text>
      </View>
      {weeklyReading.length !== 0 ?
        <ReadingChart 
          weeklyReadingData={calculateWeeklyReading(currentWeekStart, readingData)}
        /> :
        <Loader />
      }
      <View style={readingTrackerStyles.weekChangeButtonView}>
        <Button onPress={() => changeWeek('previous', currentWeekStart, setCurrentWeekStart, setWeekEnd)} > 
          Previous Week
        </Button>
        <Button onPress={() => changeWeek('next', currentWeekStart, setCurrentWeekStart, setWeekEnd)} > 
          Next Week
        </Button>
      </View>
      <Button onPress={() => setPagesDialogVisible(true)} mode='outlined' style={{marginBottom: 10}} > 
        Enter what you read
      </Button>
      <View>
